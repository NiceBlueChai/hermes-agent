...
class PrepareBootstrapToolsTests(unittest.TestCase):
    ...
    def test_validate_manifest_rejects_padded_archive_name_as_unsafe(self):
        module = _load_script_module()
        root = Path("tmp-bootstrap-tools-padded-name-test")
        output_dir = root / "bootstrap-tools"
        output_dir.mkdir(parents=True, exist_ok=True)
        archive = output_dir / " uv-x86_64-pc-windows-msvc.zip"
        archive.write_bytes(b"uv archive")
    ...
    def test_validate_manifest_rejects_unknown_archive_kind(self):
        module = _load_script_module()
        root = Path("tmp-bootstrap-tools-unknown-kind-test")
        output_dir = root / "bootstrap-tools"
        output_dir.mkdir(parents=True, exist_ok=True)
        archive = output_dir / "mystery-cache-windows-x64.zip"
        archive.write_bytes(b"mystery")
        manifest = output_dir / "bootstrap-tools-manifest.json"
        manifest.write_text(
            json.dumps(
                {
        ...
        try:
            with self.assertRaisesRegex(RuntimeError, "unknown bootstrap tool archive"):
                module.validate_manifest(output_dir)
        finally:
            if archive.exists():
                archive.unlink()
    ...
    def test_signed_installer_workflows_print_fallback_evidence_commands(self):
        ...
        for workflow_text in (windows_workflow, unix_workflow):
            ...
            self.assertIn("source archive must use NAME=HTTPS_URL=SHA256", workflow_text)
            ...
            self.assertIn("--print-status", workflow_text)
            self.assertIn("--add-evidence-json", workflow_text)
            self.assertIn("-status.json", workflow_text)
            self.assertIn("desktop-bootstrap-script-fallback", workflow_text)
            self.assertIn("installer-source-archive-download-fallback", workflow_text)
            self.assertIn("installer-python-runtime-download-fallback", workflow_text)
            self.assertIn(".release-evidence/fallback-burn-down", workflow_text)
