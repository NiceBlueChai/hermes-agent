...
- Native bridge returns structured stage events and failure categories. **Started: `hermes-manager bootstrap-capabilities`
  ...
  skills fallback copy. Windows non-interactive `configure` and `gateway` stages now short-circuit through native skip
  results instead of launching PowerShell only to no-op. Desktop keeps script fallback when full native bootstrap is not
  available or the native stage returns a known fallback category.**
