...
- Native bridge returns structured stage events and failure categories. **Started: `hermes-manager bootstrap-capabilities`
  ...
  skills fallback copy. Windows `platform-sdks` now has a native probe that skips safely when no messaging SDK recovery
  is needed and falls back to the script when configured tokens require real import/install checks. Windows
