...
- Rust bootstrap archive resolution now requires manifest `sizeBytes` before trusting bundled files or cache checksum
  metadata, matching the stricter script recovery path.
