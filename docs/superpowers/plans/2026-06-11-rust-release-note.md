...
- If native Python dependency setup falls back to the install scripts, the Rust bootstrapper now passes its bundled
  wheelhouse path through so script recovery can still avoid PyPI when the release package contains valid wheels.
