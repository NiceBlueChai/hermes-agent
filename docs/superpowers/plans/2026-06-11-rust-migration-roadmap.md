...
- Script fallback for Python dependency and platform SDK recovery stages now receives the Tauri bundled wheelhouse path
  from the Rust bootstrapper, so fallback retries can still use release-bundled wheels before network PyPI tiers.
