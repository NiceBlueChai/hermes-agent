...
- `desktop` is now also excluded from the script fallback budget when desktop packaging is not requested, so the plan
  summary matches the execution loop for non-desktop bootstrap flows.
...
- Direct script cache recovery now validates those bundled cache archives against `bootstrap-tools-manifest.json`
  size and SHA-256 metadata before extracting them, preserving network fallback for missing or mismatched archives.
- The Rust installer self-check now also validates that `tauri.conf.json` still declares `bootstrap-tools/` and
  `wheelhouse/` as bundled resources, so release smoke tests catch accidental removal before upload.
...
- Native platform SDK recovery and script fallback stages now receive the Tauri bundled wheelhouse path from the Rust
  bootstrapper, so recovery retries can still use release-bundled wheels before network PyPI tiers.
...
- Shell-script cache-archive fallback now rejects blank archive member names before extraction, matching the Rust
...
- Python wheelhouse release validation and the Rust runtime wheelhouse reader now reject blank or padded wheel names,
  keeping bundled dependency resources under direct manifest-owned file names.
- Built-installer self-check now uses the same blank/padded filename guard for bootstrap-tools, wheelhouse wheels, and
